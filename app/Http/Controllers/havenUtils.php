...
class havenUtils extends Controller
{
    ...
    public static function studentProgress($studentId, $asJson = false)
    {
        // Validate
        if (empty($studentId)) {
            return $asJson
                ? response()->json(['error' => 'Invalid student ID'], 400)
                : 0;
        }

        // Fetch student with attendance and course lessons
        $student = Student::with(['attendance', 'course.lessons'])->find($studentId);

        if (!$student || !$student->course) {
            return $asJson
                ? response()->json(['error' => 'Student or course not found'], 404)
                : 0;
        }

        // Count lessons attended
        $lessonsCount = $student->attendance
            ? $student->attendance->groupBy('lesson_id')->map->count()
            : collect();

        $lessons = $student->course->lessons->map(function ($lesson) use ($lessonsCount) {
            $lesson->attended = $lessonsCount->get($lesson->id, 0);
            return $lesson;
        });

        $totalLessons = $lessons->count();
        $attendedLessons = $lessons->filter(fn($lesson) => $lesson->attended > 0)->count();

        $progressPercentage = $totalLessons > 0
            ? round(($attendedLessons / $totalLessons) * 100, 2)
            : 0;

        // Return either JSON or raw percentage
        if ($asJson) {
            return response()->json([
                'student' => "{$student->fname} {$student->sname}",
