...
<div class="content content-full" id="editBulkAttendance">
<div class="row">
...
<!-- END Hero -->
    ...
    <script setup>
        ...
        const editBulkAttendance = createApp({
        setup() {
            const currentDate = new Date();
            const options = { day: 'numeric', month: 'long', year: 'numeric'};
            const bulkAttendance = @json($bulkAttendance);
            const state = ref({
            ...
            onMounted(async () => {
                if (bulkAttendance) {
                    ...
                    try {
                        const lessonRes = await axios.get('/api/lessons');
                        const lessonMap = Object.fromEntries(
                            (lessonRes.data || []).map(lesson => [lesson.id, lesson.name])
                        );

                        state.value.bulkAttendanceDescription = bulkAttendance.description;

                        // Optionally set placeholder data until API loads real data
                        state.value.selectedStudents = (bulkAttendance.students ?? []).map(s => ({
                            studentId: s.id,
                            studentName: `${s.fname} ${s.mname ?? ''} ${s.sname}`.trim(),
                            lessonId: s.pivot.lesson_id,
                            lessonQuantity: s.pivot.lesson_quantity ?? null,
                            lessonName: lessonMap[s.pivot.lesson_id] ?? '',
                        }));

                        console.log(state.value.selectedStudents);
                    } catch (error) {
                        console.error('Failed to load lessons or students:', error);
        ...
        editBulkAttendance.mount('#editBulkAttendance')
    </script>
@endsection
